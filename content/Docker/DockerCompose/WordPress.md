...
- `ports:` Пробрасывает порт 80 из контейнера WordPress на порт 8080 вашего компьютера. Вы можете изменить внешний порт, например, на 8000:80, если порт 8080 уже занят.
...
параметр `-d` означате фоновый режим запуска контейнеров
...
### 4. Проверить установку WP-приложения
...
docker compose down
```
Остановка с полным удалением всех данных (базы данных и файлов) - опционально:
```shell
docker compose down --volumes
```
(**Будьте осторожны:** эта команда удалит всё, что вы создали на сайте Word Press!).

> ### Для полного удаления этого проекта, достаточно остановить его через `docker compose down` или `docker compose down --volumes` и docker-образ, после чего удалить каталог `wordpress`
